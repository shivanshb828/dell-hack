...
def _speaking_chunk(audio: bytes) -> dict:
    """Mock VAD chunk with explicit rms/confidence so thresholds pass."""
    return {
        "is_speaking": True,
        "speech_ended": False,
        "audio": audio,
...
def test_interruption_tracker_sets_stop_event_and_accumulates_audio():
    ...
    min_chunks = pipeline.INTERRUPT_MIN_SPEECH_CHUNKS  # currently 5

    vad = MagicMock()
    speaking_side_effects = [
        _speaking_chunk(f"audio-{i}".encode()) for i in range(min_chunks)
    ]
    vad.process.side_effect = speaking_side_effects + [
        {"is_speaking": False, "speech_ended": True, "audio": b"final\x00"},
    ]

    # Feed (min_chunks - 1) chunks — not yet triggered
    for i in range(min_chunks - 1):
        result = tracker.feed(vad=vad, chunk=f"chunk-{i}".encode(), stop_event=stop_event)
        assert result is False
        stop_event.set.assert_not_called()
        assert tracker.interrupted is False

    # Feed the threshold chunk — now triggers
    result = tracker.feed(vad=vad, chunk=b"chunk-threshold", stop_event=stop_event)
    assert result is False
    stop_event.set.assert_called_once()
    assert tracker.interrupted is True

    # Feed end-of-speech chunk — returns True
    result = tracker.feed(vad=vad, chunk=b"chunk-end", stop_event=stop_event)
    assert result is True
    assert tracker.audio == b"final\x00"
